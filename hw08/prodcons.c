...
[Description]  : 
    - 생산자와 소비자 문제를 해결하기 위한 프로그램.
    - 생산자는 데이터를 생성하여 공유 버퍼에 추가, 소비자는 공유 버퍼에서 데이터를 소비.
    - 세마포어를 사용하여 동기화 및 상호 배제를 구현.
[Input]        :
    sem_t EmptySem;  // 버퍼의 빈 공간을 관리하는 세마포어
    sem_t FullSem;   // 버퍼의 데이터 개수를 관리하는 세마포어
    sem_t MutexSem;  // 공유 자원의 동시 접근을 방지하는 세마포어
[Output]       :
